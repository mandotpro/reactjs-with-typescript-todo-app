        ...
        return (
            <ul style={this._stylesUL}> {
                    this.filters.map((item, index) => {
                        return <li key={index} onClick={this.props.onFilter.bind(null, item.filter)} data-test-id={item.filter} style={ item.filter == this.props.filter ? activeEl : this._stylesLI}>{ item.text }</li>
                    })
                }
            </ul>
        )
